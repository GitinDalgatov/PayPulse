...
public class WalletService {
    ...
    public WalletService(WalletRepository walletRepository, HistoryRepository historyRepository, OutboxService outboxService, AuditService auditService) {
    ...
    private void publishBalanceEvent(UUID userId, String type, String description) {
        BalanceChangedEvent event = new BalanceChangedEvent(userId, type, description);
        outboxService.saveBalanceEvent(userId.toString(), event);
    }

    private void updateWalletBalance(UUID userId, BigDecimal amount, String description, String operationType) {
    ...
    public Page<HistoryResponse> getHistory(Pageable pageable) {
        UUID userId = getCurrentUserId();
        Page<TransactionHistory> historyPage = historyRepository.findByUserIdOrderByTimestampDesc(userId, pageable);
        return historyPage.map(tx -> new HistoryResponse(tx.getAmount(), tx.getDescription(), tx.getTimestamp()));
    }

    public Page<HistoryResponse> getAllHistory(Pageable pageable) {
        Page<TransactionHistory> historyPage = historyRepository.findAll(pageable);
        return historyPage.map(tx -> new HistoryResponse(tx.getAmount(), tx.getDescription(), tx.getTimestamp()));
    }
