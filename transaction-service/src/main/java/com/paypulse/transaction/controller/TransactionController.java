...
public class TransactionController {
    ...
    @Operation(summary = "Создать транзакцию", description = "Создает новую транзакцию между пользователями")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Транзакция создана", content = @Content(schema = @Schema(implementation = TransactionResponse.class))),
        @ApiResponse(responseCode = "400", description = "Ошибка валидации")
    })
    @PostMapping
    public ResponseEntity<com.paypulse.common.TransactionResponse> create(
    ...
    @Operation(summary = "История транзакций пользователя", description = "Возвращает историю транзакций пользователя")
    @ApiResponse(responseCode = "200", description = "История получена", content = @Content(schema = @Schema(implementation = TransactionResponse.class)))
    @GetMapping({"/", "/history", ""})
    public ResponseEntity<List<com.paypulse.common.TransactionResponse>> getHistory() {
    ...
    public ResponseEntity<Object> getOutboxMetrics() {
